...
public class OrderManager {
     ...
     * @throws Exception
     */
    private SubmitOrderParams createSumitOrderParams(String privateKey,
                                                    OrderInfo orderInfo,
        ...
        return new SubmitOrderParams(
                orderInfo.address, orderInfo.protocolS, orderInfo.protocolB, amountHexS, amountHexB,
     ...
     */
    public void submitOrder(OrderInfo orderInfo, String privateKey, String orderWalletAddress, LooisListener listener) {
        Flowable.just(privateKey)
                .flatMap((Function<String, Flowable<LooisSubmitOrder>>) s -> {
                    SubmitOrderParams sumitOrderParams = createSumitOrderParams(privateKey, orderInfo,
                            true, 50, orderWalletAddress);
                    LooisSubmitOrder looisSubmitOrder = Loois.client().looisSubmitOrder(sumitOrderParams).sendAsync().get();
